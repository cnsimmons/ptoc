...
exp = sys.argv[1] #which experiment
sub = sys.argv[2] #WHICH SUBJECT
...
#instatiate sub dir
sub_dir = f'{study_dir}/{sub}/ses-{ses:02d}'
...
if not os.path.exists(f'{sub_dir}/anat/{sub}_ses-{ses:02d}_T1w_brain.nii.gz'):
    #deskull brain
    bash_cmd = f'bet {anat_file} {sub_dir}/anat/{sub}_ses-{ses:02d}_T1w_brain.nii.gz -R -B'
...
for task in tasks:
    ...
    for run in runs:
        #if bold file exists
        if os.path.exists(f'{sub_dir}/func/{sub}_ses-{ses:02d}_task-{task}_run-0{run}_bold.nii.gz'):
            #create run directory
            os.makedirs(f'{sub_dir}/derivatives/fsl/{task}/run-0{run}', exist_ok=True)

            #calcualte motion outliers
            bash_cmd = f'fsl_motion_outliers -i {sub_dir}/func/{sub}_ses-{ses:02d}_task-{task}_run-0{run}_bold.nii.gz -o {sub_dir}/derivatives/fsl/{task}/run-0{run}/{sub}_ses-{ses:02d}_task-{task}_run-0{run}_bold_spikes.txt --dummy=0'
            subprocess.run(bash_cmd.split(),check = True)
            #print(bash_cmd)
