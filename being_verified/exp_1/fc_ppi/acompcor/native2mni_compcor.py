...
for sub in subs:
    ...
    for roi in ['LO','pIPS']:
        for hemi in ['left', 'right']:
            ...
            # --- ORIGINAL PPI to MNI (commented out) ---
            #ppi_native = f"{out_dir}/fc/{sub}_{roi}_{hemi}_loc_ppi.nii.gz"
            #ppi_mni = f"{out_dir}/fc_mni/{sub}_{roi}_{hemi}_loc_ppi_mni.nii.gz"

            # --- ACOMPCOR PPI to MNI ---
            ppi_native = f"{out_dir}/ppi/{sub}_{roi}_{hemi}_loc_ppi_acompcor.nii.gz"
            ppi_mni = f"{out_dir}/fc_mni/{sub}_{roi}_{hemi}_loc_ppi_acompcor_mni.nii.gz"

            if os.path.isfile(ppi_native) and not os.path.isfile(ppi_mni):
                print(f"Registering aCompCor PPI for {sub}, ROI {roi}, Hemisphere {hemi} to MNI space")
                subprocess.run([
                    'flirt',
                    '-in', ppi_native,
                    '-ref', mni_brain,
                ...
                print(f"aCompCor PPI MNI file already exists for {sub}, ROI {roi}, Hemisphere {hemi}")
            else:
                print(f"aCompCor PPI file not found for {sub}, ROI {roi}, Hemisphere {hemi}")
