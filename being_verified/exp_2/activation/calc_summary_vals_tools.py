...
def calc_summary_vals(sub, task, cope, roi, hemi):
    ...
    func = image.load_img(f'{raw_dir}/{sub}/ses-01/derivatives/fsl/{task}/HighLevel.gfeat/cope{cope}.feat/stats/zstat1.nii.gz')
    ...
    if roi != 'hemi':
        #load the roi
        roi = image.load_img(f'{raw_dir}/{sub}/ses-01/derivatives/rois/parcels/{roi}.nii.gz')
...
for sub, group, hemi in zip(sub_info['sub'], sub_info['group'], sub_info['intact_hemi']):
    ...
    #check if sub already has 'sub-' prefix
    if sub[:4] != 'sub-':
        sub = 'sub-' + sub
    ...
    for hemi in hemis:
        for roi in rois:
            for task, cond, cope in zip(task_info['task'], task_info['cond'], task_info['cope']):
                ...
                if os.path.exists(f'{raw_dir}/{sub}/ses-01/derivatives/fsl/{task}/HighLevel.gfeat'):
                    ...
                    summary_df = summary_df.append({'sub': sub, 
                                                  'group': group, 
                                                  'hemi': hemi, 
                                                  'roi': roi, 
                                                  'cond': cond, 
