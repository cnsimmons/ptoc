# Prepare overlap data for surface visualization with PyCortex for experiment 2
import os
import numpy as np
...
group_out_dir = f'{results_dir}/group_averages_exp2'  # Updated for exp2
surface_out_dir = f'{results_dir}/surface_maps_exp2'  # Updated for exp2

# Create surface output directory if it doesn't exist
os.makedirs(surface_out_dir, exist_ok=True)

def prepare_overlap_data(analysis_type='fc', hemisphere='both', condition='tools', subject='fsaverage', xfmname='atlas_2mm'):
    """
    Prepare overlap data for surface visualization using PyCortex for experiment 2.
    ...
    condition : str
        Which condition to process ('tools' or 'nontools')
    subject : str
        Subject ID for PyCortex
    xfmname : str
        Transform name for PyCortex
    """
    print(f"Preparing {analysis_type.upper()} analysis for {condition} condition, surface visualization...")
    ...
    for hemi in hemispheres:
        print(f"\nProcessing {hemi} hemisphere for {condition} condition...")
        
        # Define file paths - updated to include condition
        pips_img_path = f'{group_out_dir}/pIPS_{hemi}_{condition}_{analysis_type}_thresh.nii.gz'
        lo_img_path = f'{group_out_dir}/LO_{hemi}_{condition}_{analysis_type}_thresh.nii.gz'
        
        # Check if files exist
        if not os.path.exists(pips_img_path) or not os.path.exists(lo_img_path):
            print(f"Warning: Files not found for {hemi} hemisphere, {condition} condition, {analysis_type}. Skipping.")
            continue
        ...
        # Save the resampled NIfTI files for surface visualization - updated to include condition
        overlap_file = f'{surface_out_dir}/overlap_{condition}_{analysis_type}_{hemi}.nii.gz'
        overlap_only_file = f'{surface_out_dir}/overlap_only_{condition}_{analysis_type}_{hemi}.nii.gz'
        pips_only_file = f'{surface_out_dir}/pips_only_{condition}_{analysis_type}_{hemi}.nii.gz'
        lo_only_file = f'{surface_out_dir}/lo_only_{condition}_{analysis_type}_{hemi}.nii.gz'
        ...
        print(f"Saved files for {hemi} hemisphere, {condition} condition:")
        ...
        # Print statistics
        print(f"\nOverlap Statistics for {analysis_type.upper()}, {condition} condition, {hemi} hemisphere:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Prepare overlap data for surface visualization for experiment 2')
    ...
    parser.add_argument('--condition', '-c', type=str, choices=['tools', 'nontools', 'both'], default='both',
                      help='Condition to prepare (tools, nontools, or both)')
    ...
    for condition in conditions:
        for analysis_type in analysis_types:
            prepare_overlap_data(analysis_type=analysis_type, hemisphere=args.hemisphere, condition=condition)
