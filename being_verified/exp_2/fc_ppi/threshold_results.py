this does not include the tools vs nontools contrast see '''below'''
...
'''
# edits to include tools over nontools
# # Updated threshold_results.py for experiment 2 with tools vs nontools contrast
import os
import numpy as np
import pandas as pd
import nibabel as nib
...
def main():
    ...
    rois = ['pIPS', 'LO']
    hemispheres = ['left', 'right']
    conditions = ['tools', 'nontools', 'tools_vs_nontools']  # Added contrast condition
    analysis_types = ['fc', 'ppi']
    ...
    # Process each condition, analysis type, roi, and hemisphere
    for condition in conditions:
        # Only process PPI for tools_vs_nontools contrast
        analysis_types_to_process = ['ppi'] if condition == 'tools_vs_nontools' else analysis_types
        
        for analysis_type in analysis_types_to_process:
            # Find global max for consistent visualization
            global_vmax = 0
            ...
            for roi in rois:
                ...
                for i, hemi in enumerate(hemispheres):
                    ...
                    if condition == 'tools':
                        ...
                        # For nontools, include condition in file name
                        file_pattern = f'{study_dir}/{{sub}}/ses-01/derivatives/{analysis_type}/mni/{{sub}}_{roi}_{hemi}_{condition}_ToolLoc_{analysis_type}_mni.nii.gz'
                    else:  # tools_vs_nontools
                        # For tools vs nontools contrast
                        file_pattern = f'{study_dir}/{{sub}}/ses-01/derivatives/{analysis_type}/mni/{{sub}}_{roi}_{hemi}_{condition}_ToolLoc_{analysis_type}_mni.nii.gz'
                    ...
                    def find_file(sub, pattern):
                        ...
                        if condition == "nontools":
                            condition_part = "_nontools"
                        elif condition == "tools_vs_nontools":
                            condition_part = "_tools_vs_nontools"
                            
                        alternatives = [
                            f'{study_dir}/{sub}/ses-01/derivatives/{analysis_type}/mni/{sub}_{roi}_{hemi}{condition_part}_ToolLoc_{analysis_type}_mni.nii.gz',
                            f'{study_dir}/{sub}/derivatives/{analysis_type}/mni/{sub}_{roi}_{hemi}{condition_part}_ToolLoc_{analysis_type}_mni.nii.gz',
                            f'{study_dir}/{sub}/ses-01/derivatives/{analysis_type}/{sub}_{roi}_{hemi}{condition_part}_ToolLoc_{analysis_type}_mni.nii.gz'
                        ]
