...
## TEMP CONVERSION TO STANDARD SPACE
...
for sub in tqdm(subs, desc="Converting to MNI space"):
    ...
    analysis_mni_dir = f"{out_dir}/{analysis_type}_mni"
    os.makedirs(analysis_mni_dir, exist_ok=True)
    ...
    for hemi in ['left', 'right']:
        input_file = f"{analysis_dir}/{sub}_LO_minus_pIPS_{hemi}_{analysis_type}.nii.gz"
...
print(f"Conversion to MNI space completed for all subjects using {analysis_type} analysis.")
...
''''
# Conversion to Standard Space || this is functional but want to update loop to include ppi and fc
